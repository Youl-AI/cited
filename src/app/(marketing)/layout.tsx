 ...
 * `dark:` 변형이 깨어난다. 앱(`(app)`)·리포트(`audit`)·약관(`legal`)은
 * 라이트 그대로다 — 페이지 중간에서 테마가 뒤집히는 것이 아니라 **표면
 * 단위로 갈린다**(tasteskill §4.11 Page Theme Lock).
 *
 * ★ 래퍼는 `min-h-dvh`여야 한다. body는 여전히 라이트 배경이라 래퍼가 짧으면
 *   아래쪽에 흰 띠가 남는다.
 * ★ `overflow-x-clip`이지 `overflow-x-hidden`이 아니다. hidden은 이 요소를
 *   **스크롤 컨테이너로 만든다** — 그러면 Task 4가 붙일 ScrollTrigger 핀이
 *   뷰포트가 아니라 이 컨테이너를 기준으로 잡히고(핀이 어긋나는 대표적인
 *   원인), sticky도 함께 어긋난다. clip은 잘라내기만 하고 스크롤 컨테이너를
 *   만들지 않는다.
 *
 * ## 왜 `SiteShell`을 쓰지 않는가
 *
 * `SiteShell`은 앱 머리글(`SiteHeader`)을 세운다. 마케팅 표면은 떠 있는 유리
 * 알약(`MarketingHeader`)을 쓰고, 그것 말고는 셸이 하는 일이 같다.
 * `audit`·`legal`은 계속 `SiteShell`이다.
 */
