 ...
 *
 * ★ 이것은 **통계적으로 최적인 검정이 아니다. 일부러 그렇게 했다.**
 *
 *   두 비율이 다른지 엄밀히 보려면 2-표본 비율 검정(또는 차이의 신뢰구간)을
 *   써야 한다. 구간 겹침 판정은 그보다 **보수적**이라, 두 95% 구간이 겹치는데도
 *   차이 검정으로는 유의한 경우가 존재한다. 즉 이 함수는 **실제 변화를 놓칠 수 있다.**
 *
 *   그럼에도 이걸 쓰는 이유는 제품 결정이다. 이 판정의 출력은 대시보드의 ▲▼이고,
 *   고객은 그 화살표를 보고 마케팅 예산을 움직인다. 없는 변화를 있다고 말하는
 *   비용이 있는 변화를 놓치는 비용보다 훨씬 크다. 한 번 헛짚으면 그다음부터
 *   모든 숫자를 의심받는다.
 *
 *   **그러니 "통계적으로 부정확하다"는 이유로 2-표본 검정으로 바꾸지 마라.**
 *   바꾸려면 그건 통계 수정이 아니라 **제품 정책 변경**이고, 오탐이 얼마나 늘지
 *   골든 라벨이나 실제 수집 데이터로 먼저 재고 나서 결정해야 한다.
 */
