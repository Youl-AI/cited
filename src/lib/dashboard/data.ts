...
export interface SourceChangeRow {
   ...
   *
   * ★ false면 화면은 **직전 회차에 대한 어떤 주장도** 하면 안 된다 —
   *   `prevAnswers → answers` 화살표도, "새로 등장"도. 질의를 셋 더 넣은 다음
   *   회차는 출처 인용 수가 당연히 늘고, 판정기가 바뀌면 무엇을 인용으로
   *   셌는지가 바뀐다 — "2 → 5"는 브랜드가 한 일이 아니라 설정 변경이다.
   *   "새로 등장"도 "직전 회차에는 없었다"는 직전 회차 비교라 같은 규칙을
   *   받는다: 질의가 갈린 경계에서 상위 도메인이 물갈이되면, 그 물갈이는
   *   브랜드의 성과가 아니라 설정 변경이다. 언급률 추이는 `comparableWithPrev`로
   *   선을 끊는데 출처 표만 비교 서사를 쓰면, 같은 거짓말이 표 모양으로 나갈
   *   뿐이다. 화면은 개수만 쓴다.
   *
   * ★ 직전 회차가 아예 없으면 false다 — 비교 자체가 없다. (이 경우에도 화면은
   *   개수만 쓴다 — 없는 회차를 두고 "새로 등장"도 "조건이 달라"도 말이 안 된다.)
   *   그래도 `prevAnswers`를 null로 뭉개지 **않는** 이유: "직전에 없던
   *   도메인"과 "비교할 수 없는 회차"는 다른 사실이고, null 하나로 합치면
   *   비교 가능한 경계에서 멀쩡히 있던 도메인이 "새로 등장"으로 나가게 된다.
   */
  comparableWithPrev: boolean
}
