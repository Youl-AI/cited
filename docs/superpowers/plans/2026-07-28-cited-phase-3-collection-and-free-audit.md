...
> ### ★ 2026-07-30(3) — 실행 완료. 디자인 방향과 브라우저에서 잡은 것
>
> **서명 요소는 `AnswerSpecimen`이다.** 히어로가 큰 숫자나 대시보드 목업이
> 아니라 **실제 AI 답변 원문**이다. 이 제품이 파는 것은 대시보드가 아니라
> 내가 통제할 수 없는 남의 문장이고, GEO를 모르는 사람의 장벽은 "무엇을
> 받나"가 아니라 "왜 신경 써야 하나"다. 랜딩과 리포트가 **같은 컴포넌트**를
> 쓴다 — 보여준 것과 배송물이 같아야 "이거 진짜야?"에 답이 된다.
>
> 표시 규칙이 제품의 한계 설명을 대신한다: **등록한 브랜드만 밑줄**이 그어지고
> W컨셉·쿠팡은 평문으로 남는다. 그게 사실이고(등록 안 한 브랜드는 셀 수 없다),
> Share of Voice의 가장 중요한 주의사항을 문단 없이 가르친다.
> 순서 번호는 자기 브랜드에만 붙인다 — 경쟁사의 답변별 순서는 재지 않는다.
>
> **조판 규칙: sans는 말, mono는 계측값.** 다만 mono에는 **한글 글리프가 없다.**
> `3개`·`무제한`·`1 / 2 단계`처럼 한글이 섞인 곳에 `font-mono`를 걸면 글자마다
> 시스템 서체로 떨어져 한 줄에서 서체가 갈린다(브라우저에서 실제로 그렇게
> 보였다). **숫자만 감싼다.**
>
> **브라우저에서 잡은 실제 버그 세 개** (`dev-browser`로 렌더해서 확인):
> 1. **내부 식별자 노출** — 랜딩에 "무료 진단은 chatgpt · gemini만 봅니다"가
>    찍혀 나갔다. `PLANS.free.engines`를 그대로 join한 결과다.
>    `ENGINE_LABEL`·`engineLabel()`을 `plans.ts`에 두고 랜딩·요금제·리포트
>    화면·리포트 메일 **네 곳 모두** 고쳤다.
> 2. **zod 기본 영문 메시지 노출** — 잘못된 이메일을 넣으니 폼에
>    `Invalid email address`가 떴다. 라우트가 첫 이슈 메시지를 그대로 응답에
>    담고 폼이 그대로 띄우기 때문이다. `request-schema.ts`의 메시지를 전부
>    한국어로 주고, 라틴 문자만으로 된 메시지가 새어 나오면 실패하는 테스트를
>    붙였다.
> 3. **계획서 Step 3 테스트의 기대값이 틀렸다** — `2% ~ 87%`를 기대하는데
>    이 저장소의 Wilson score 구간은 k=1·n=3에서 `6% ~ 79%`다. 값을 손으로
>    적지 않고 `formatInterval`로 유도하도록 바꿨다.
>
> **그 밖에:**
> - `EvidenceItem.position`을 추가했다. 이미 재고 있던 값인데 리포트에 담기지
>   않아 랜딩에만 순서 번호가 있었다.
> - `SiteShell`을 뽑았다. `/audit/*`가 `(marketing)` 밖이라 헤더·푸터가 없었고,
>   푸터에는 전자상거래법 제10조가 요구하는 사업자 표시가 들어 있다.
> - `jsdom`은 **29.1.1로 고정**했다. 30.0.1은 Node `^24.15.0`을 요구하는데
>   이 저장소는 24.13.1이다.
> - 컴포넌트 테스트만 파일 상단 `// @vitest-environment jsdom`으로 전환한다.
>   `environmentMatchGlobs`는 vitest 3에서 폐기됐다.
> - 변이 14건 전부 잡힘.
