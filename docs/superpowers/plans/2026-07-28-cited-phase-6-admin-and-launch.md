...
**먼저 볼 것은 상한이 아니라 예산 킬스위치다.** 원가가 정말 문제라면
`MONTHLY_BUDGET_KRW`가 이미 막고 있어야 한다. 안 막고 있다면 킬스위치가
고장난 것이고, 그게 진짜 사고다 — 건수 상한을 만지기 전에 그걸 확인한다.

킬스위치는 멀쩡한데 예산을 너무 빨리 태우는 것이라면:

```ts
// src/lib/audit/limits.ts
export const DAILY_GLOBAL_LIMIT = 20   // 일일 유입 속도를 늦춘다
export const MONTHLY_BUDGET_KRW = 100_000  // 월 총액 자체를 조정한다
```

상한 소진 시 에러가 아니라 대기 등록으로 받으므로 리드는 계속 확보된다.

**무료 진단은 트래픽이 늘수록 순수 적자다.** 건당 약 100원이므로
일 20건 = 월 600건 = 변동비 약 6만원이고, 월 예산 상한은 10만원이다.
(최초 계획서는 일 100건으로 잡혀 있었다 — 건당 50~110원이라는 틀린 원가에서
나온 값이고, 실제로는 월 30만원이 되어 예산의 3배다.)

**그리고 급증이 사람인지 봇인지부터 가른다.** Turnstile이 켜져 있는데도
급증했다면 사람 트래픽일 가능성이 높고, 그건 막을 게 아니라 좋은 신호다 —
그때는 상한을 낮추는 게 아니라 **예산을 올리는 판단**을 해야 한다.
...
| 항목 | 설계 추정 | 실측 |
| --- | --- | --- |
| 무료 진단 1건 원가 (6호출, 저가 모델) | 80~125원 | __원 |
| 유료 측정 호출 1회 원가 (고급 모델) | 39~60원 | __원 |
| 웹검색 호출의 입력 토큰 | 8,000 (가정) | __ 토큰 |
| Starter 월 원가 | 11,300원 | __원 |
| Starter 원가율 | 11% | __% |
| Business 월 원가 | 34,000원 | __원 |
| Business 원가율 | 12% | __% |
| 무료 진단 월 실지출 | ≤ 100,000원 (예산 상한) | __원 |
| 1차 판정 통과율 | 20~30% | __% |
| 월 고정비 | 68,000원 | __원 |
| 토스 수수료율 | 3% | __% |
