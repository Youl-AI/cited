...
> **라벨 200건을 어디서 구하나 (2026-07-29 추가)**
>
> 이 계획은 200건을 손으로 라벨링하라고만 하고 그 답변을 어디서 가져올지는
> 정하지 않았다. 답은 **크몽 수동 진단 주문**이다 — [kmong-listing.md](
> ../../kmong-listing.md)의 "Cited 개발과 맞물리기" 참고.
>
> 주문 한 건이 DELUXE 기준 답변 100건을 만들고, 라벨링은 리포트를 검수하며
> 어차피 눈으로 보는 작업이라 추가 비용이 거의 없다. **주문 2~3건이면 채워진다.**
> 직접 만든 표본보다 실제 고객 브랜드·실제 업종이라 판정기 검증에 훨씬 낫다.
> Step 1의 후보 수집 스크립트를 주문 처리 흐름에 맞춰 쓰면 된다.
>
> 같은 이유로 **`scripts/report.ts`(브랜드·질의 → 리포트 PDF)를 이 단계에서
> 같이 만들 것을 권한다.** 이 단계의 `engines/` `detection/` `stats/`를 그대로
> 쓰므로 새로 만들 것은 조립과 출력뿐이고, 3단계 무료 진단 리포트에 재사용된다.
> 없으면 주문 한 건에 2~3시간이 들어 개발 시간을 잠식한다.
