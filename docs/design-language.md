...
- **지표 상태 5색은 채우기 전용,** 텍스트·아이콘은 `-fg` 짝을 쓴다
  ...
  `text-metric-up-fg`. 예외 하나: `--color-ci-band`는 면으로만 쓰여 `-fg` 짝이
  없다 — 띠 위에 글자를 올릴 때는 `--foreground`를 쓴다(`globals.css` 주석).
...
- 끊는 규칙은 추이 차트와 같다: `comparableWithPrev === false` 또는
  `runsSkippedBefore > 0`이면 선분과 밴드를 잇지 않고 캡션에 이유를 쓴다.
  단 SoV의 `comparableWithPrev`는 추이가 보는 셋(엔진·질의 집합·판정기 버전)
  **위에 경쟁사 집합까지** 본다.
...
- **움직이는 것:** opacity·transform(translate/scale)·color 계열
  (color·background-color·border-color)만. 레이아웃 속성
  (width·height·top·margin)은 애니메이션하지 않는다.
